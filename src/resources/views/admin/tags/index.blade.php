...
<body>
    <div class="structure">
        ...
        <div class="content-display">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h1">Tags</h1>
                <a href="{{ route('admin.handle.view', ['model' => 'tags', 'action' => 'create']) }}" class="btn btn-success">
                    <i class="bi bi-plus-circle me-1"></i> Agregar nuevo tag
                </a>
            </div>
            ...
            <!-- Buscador -->
            <div class="mb-3">
                <input type="text" id="searchInput" class="form-control" placeholder="Buscar tag...">
            </div>
                    ...
                    <tbody id="category-table-body">
                        @forelse ($records as $record)
                            <tr class="category-row">
                                ...
                                <td>{{ $record->name }}</td>
                                <td>
                                    <a href="{{ route('admin.handle.view', ['model' => 'tags', 'action' => 'edit', 'target' => $record->id]) }}" class="btn btn-sm btn-warning me-1">
                                        <i class="bi bi-pencil-square"></i> Editar
                                    </a>

                                    <form action="{{ route('admin.handle.delete', ['model' => 'tags', 'target' => $record->id]) }}" method="POST" class="d-inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este tag?')">
                                        @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-muted text-center">No hay tags registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <script>
        ...
        const baseUrl       = "{{ route('admin.handle.view', ['model' => 'tags']) }}";
